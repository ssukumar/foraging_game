...
har_dur_df =har_dur_df_lf # rbind(har_dur_df, har_dur_df_lf)
...
har_dur_df = har_dur_df %>% group_by(subject, tree_type, env) %>% mutate(tree_id_by_type = row_number())
...
ggplot(data= har_dur_df_lf) + geom_line(aes(x = tree, y = num_harvests, color = env))+
  geom_point(aes(x= tree, y = num_harvests, shape = tree_type, color = env), size= 2)+
  facet_wrap(~subject, nrow=3, scales = "free_x")+
  xlab("Tree") + ylab("# of Harvests")+
  ggtitle("Long Travel First")+
  theme_classic()

ggplot(data= har_dur_df_lf) + geom_line(aes(x = tree_id_by_type, y = num_harvests, color = env, linetype = tree_type))+
  geom_point(aes(x = tree_id_by_type, y = num_harvests, shape = tree_type, color = env), size= 2)+
...
ggplot(data= har_dur_df_lf) + geom_line(aes(x = tree, y = difficulty_at_exit, color = env, linetype=tree_type))+
  facet_wrap(~subject, nrow=3, scales = "free")+
  xlab("Tree") + ylab("Difficulty at exit")+
  ggtitle("Long Travel First")+
  theme_classic() 
...
ggplot(data=har_dur_agg) + geom_line(aes(x = tree_type, y = mean_num_harvests, color = env, group = env), width = 0, position = position_dodge(width=0.2))+
...
```{r}
ggplot(data=har_dur_agg) + geom_line(aes(x = tree_type, y = mean_harvest_dur, color = env, group = env), width = 0, position = position_dodge(width=0.2))+
...
ggplot(data=har_dur_agg_norm) + geom_line(aes(x = tree_type, y = mean_norm_harvest_dur, color = env, group = env), position = position_dodge(width=0.2))+
...
ggplot(data=har_dur_agg_norm) + geom_line(aes(x = tree_type, y = mean_norm_num_harvests, color = env, group = env), position = position_dodge(width=0.2))+
  geom_errorbar(aes(x = tree_type, ymin = norm_harvest_dur_min, ymax = norm_harvest_dur_max, color =env), width = 0, position = position_dodge(width=0.2))+
  xlab("Tree Type") + ylab("Normalized Harvest Duration")+
    ggtitle("Short Travel First")+
  theme_classic()+
